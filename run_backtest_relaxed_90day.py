...
=================================
Testing with looser RSI/Z-score filters on full 90-day data
"""
...
print("=" * 70)
print("RELAXED FILTERS + 90-DAY BACKTEST")
print("=" * 70)
...
CONFIG = {
    # RELAXED Entry filters (less strict than before)
    'rsi_oversold': 35,        # Was 30 - now more relaxed
    'rsi_overbought': 65,     # Was 70
    'zscore_oversold': -1.5,  # Was -2.0 - now more relaxed
    'zscore_overbought': 1.5, # Was 2.0
    'volume_ratio_min': 1.0,   # Was 1.2 - now more relaxed
    
    # Risk Management (same as optimized)
...
print(f"   RSI oversold: {CONFIG['rsi_oversold']} (was 30)")
print(f"   Z-score oversold: {CONFIG['zscore_oversold']} (was -2.0)")
print(f"   Volume ratio: {CONFIG['volume_ratio_min']} (was 1.2)")
print(f"   SL: {CONFIG['stop_loss_pct']*100:.1f}% | TP: {CONFIG['take_profit_pct']*100:.1f}%")

# ============================================================================
# DATA - USE FULL 90-DAY
# ============================================================================

print("\n[1/5] Loading full 90-day data...")
...
def generate_signal(row):
    ...
    # RELAXED: Less strict conditions
...
for ticker in sorted(df_hourly['ticker'].unique()):
    ...
    for i in range(1, len(tdf)):
        ...
        if position > 0:
            ...
            if price_change <= -sl:
                ...
                stop_losses += 1
                losses += 1
                all_trades.append({'ticker': ticker, 'entry': entry_time, 'exit': curr_time, 'pnl': gross_pnl - (position * entry_price * (commission_bps + slippage_bps) / 10000), 'exit_type': 'SL'})
                position = 0
            ...
            # Take profit
                ...
                take_profits += 1
                wins += 1
                all_trades.append({'ticker': ticker, 'entry': entry_time, 'exit': curr_time, 'pnl': gross_pnl - (position * entry_price * (commission_bps + slippage_bps) / 10000), 'exit_type': 'TP'})
                position = 0
            ...
            # Trailing stop
                ...
                trailing_exits += 1
                wins += 1
                all_trades.append({'ticker': ticker, 'entry': entry_time, 'exit': curr_time, 'pnl': gross_pnl - (position * entry_price * (commission_bps + slippage_bps) / 10000), 'exit_type': 'TS'})
                position = 0
    ...
    if position > 0:
        ...
        if final_price > entry_price:
            wins += 1
            all_trades.append({'ticker': ticker, 'entry': entry_time, 'exit': times[-1], 'pnl': gross_pnl - (position * entry_price * (commission_bps + slippage_bps) / 10000), 'exit_type': 'EOD'})
        else:
            losses += 1
            all_trades.append({'ticker': ticker, 'entry': entry_time, 'exit': times[-1], 'pnl': gross_pnl - (position * entry_price * (commission_bps + slippage_bps) / 10000), 'exit_type': 'EOD'})
...
print("\n" + "=" * 70)
print("RESULTS (RELAXED FILTERS + 90-DAY)")
print("=" * 70)
...
comparisons = [
    ...
    ("Final Optimized (5-day)", 2126),
    ("RELAXED FILTERS (90-day)", total_pnl),
]
...
with open(workspace / "reports/backtest_relaxed_90day.json", "w") as f:
    json.dump(backtest_results, f, indent=2, ensure_ascii=False)

print(f"\n✅ Results saved to reports/backtest_relaxed_90day.json")

...
@dataclass
class SklearnGradientBoostingModel(BaseTradingModel):
    model_name: str = "sklearn_gradient_boosting"
    ...
    def fit(self, rows: list[dict[str, Any]], target_key: str = "return_1") -> dict[str, float]:
        ...
        self.std_return = float(np.std(y))
    ...
    def _fallback_predict(self, rows: list[dict[str, Any]]) -> list[StandardizedPrediction]:
        ...
        std_ret = getattr(self, 'std_return', 1e-6)
        return [
            StandardizedPrediction(
                expected_return=float(mean_ret),
                direction_probability_up=clip01(0.5 + mean_ret / max(std_ret, 1e-6) * 0.1),
                direction_probability_down=clip01(0.5 - mean_ret / max(std_ret, 1e-6) * 0.1),
                confidence=clip01(abs(mean_ret) / max(std_ret, 1e-6) * 0.1),
                prediction_horizon=self.prediction_horizon,
                model_name=self.model_name,
                model_version=self.model_version,
            )
    ...
    def get_metadata(self) -> dict[str, Any]:
        ...
        if hasattr(self, 'mean_return'):
            meta["mean_return"] = float(self.mean_return)
        if hasattr(self, 'std_return'):
            meta["std_return"] = float(self.std_return)
        return meta
    ...
    @classmethod
    def load(cls, path: Path) -> "SklearnGradientBoostingModel":

...
from src.models.base.serialization import load_pickle, save_pickle


@dataclass
class StackingMetaModelPlaceholder(BaseTradingModel):
    model_name: str = "stacking_meta_placeholder"
    model_version: str = "v1"
    prediction_horizon: str = "60m"

    def fit(self, rows: list[dict[str, Any]], target_key: str = "return_1") -> dict[str, float]:
        _ = (rows, target_key)
        return {"stacking_placeholder": 1.0}
    ...
    @classmethod
    def load(cls, path: Path) -> "StackingMetaModelPlaceholder":
